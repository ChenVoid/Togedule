  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      <el-table :data="user_data" stripe style="width: 100%" border>
        ...
        <el-table-column prop="attribute" label="用户组">
        ...
        <el-table-column label="确认" width="180px">
          <template v-slot="scope">
            <!-- 修改按钮 -->
            <el-tooltip effect="dark" content="修改" placement="top" :enterable="false">
              <el-button type="primary" icon="el-icon-edit" size="mini" @click="change_manage(scope.row)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
...
      console.log(appoinmentList)
    ...
    update_account(row){
      this.$confirm('确认修改？')
          ...
          this.axios.post(url,"uid="+row.uid+"&usertype="+row.value).then(res => {
            console.log("看这里！")
            console.log(res.data)
    ...
    change_manage(row){
      this.$confirm('确认修改？')
          ...
          console.log("sbsbsb")
          console.log(typeof row.uid)
          this.axios.post(url,"uid="+row.uid+"&value="+row.value).then(res => {
            console.log("看这里！")
            console.log(res.data)
            this.res_result = res.data
            // this.$message.error(res.msg)
