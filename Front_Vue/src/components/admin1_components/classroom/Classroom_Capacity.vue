<template>
  <div>
    <p>{{this.data_test}}</p>
    <!-- 面包屑导航区域 -->
    <Breadcrumb name1="教室管理" name2="修改容量" />
    ...
    <el-card class="box-card">
      ...
      <el-table :data="data_test" stripe style="width: 100%" border>
        ...
        <el-table-column label="修改((S,L,M,E)">
...
export default {
  ...
  methods: {
    ...
    update_device(row){
      ...
      console.log(this.data_test[0]["capacity"])
      this.axios.post(url,"buildingName="+row.buildingName+"&classroomName="+row.className+"&capacity="+row.input+"&equipment="+this.data_test[0]["equipment"]+"&useinfro="+this.data_test[0]["useinfro"]+"&oprtype=edit_capacity").then(res => {
          console.log("看这里！")
