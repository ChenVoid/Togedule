<template>
  <div>
    <!-- 面包屑导航区域 -->
    <Breadcrumb name1="教室管理" name2="教室使用信息管理" />
    <!-- 卡片视图区域 -->
    <el-card class="box-card">
      ...
      <!-- <div>{{scope.row.status}}</div> -->
               <img slot="reference" :src="parseInt(scope.row.status) === 1 ?  require('../../../assets/not_empty.png'): require('../../../assets/empty.png')" style="width: 30px;height: 30px">
          </template>
        </el-table-column>
        ...
        <el-table-column prop="flow" label="人流量"></el-table-column>
        ...
        <el-table-column label="操作" width="180px">
          <template v-slot="scope">
            ...
            <el-tooltip effect="dark" content="查询" placement="top" :enterable="false">
              <el-button type="success" icon="el-icon-data-analysis" size="mini" @click="showClassroomRequire(scope.row)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    require_classroomdetails() {
      ...
      this.$router.push({ name: 'classrooms_detail', params: {json_test}})
    },

    flush(){
      console.log("flush!!!")
      var url = '/classroom/flush/';
...
          }
    ...
    showEditDialog(row){
      ...
      // console.log(this.$route.path)
      var url = '/classroom/change/';
      this.axios.post(url,"buildingName="+row.buildingName+"&classroomName="+row.classroomName+"&useInfo="+row.value).then(res => {
          console.log("看这里！")
          console.log(res.data)
          // this.$message.error(res.msg)
      }).catch(error => {
    ...
    showClassroomRequire(row) {
      ...
      console.log(row.classroomName)
      // console.log(this.$route.path)
      var url = '/appointment/select/';
      this.axios.post(url,"buildingName="+row.buildingName+"&classroomName="+row.classroomName).then(res => {
          console.log("看这里！")
          console.log(res.data)
          this.res_result = res.data
          var json_test = new Array();
...
          }
          console.log(json_test)
          console.log(json_test)
          this.$router.push({ name: 'classrooms_require', params: {json_test}})
          // this.$message.error(res.msg)
      }).catch(error => {
        console.log(error)
      })
