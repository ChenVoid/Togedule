  ...
  <div>
    <!-- <el-button type="text" @click="dialogTableVisible = true">打开嵌套表格的 Dialog</el-button> -->
      <el-dialog title="用户信息" :visible.sync="dialogTableVisible">
        <el-table :data="account_edit">
          <el-table-column property="uid" label="uid" width="150"></el-table-column>
    ...
    <el-card class="box-card">
      ...
      <el-row :gutter="10">
        ...
        <el-col :span="4">
          <el-button type="text"  @click="shuaxin()">刷新</el-button>
        </el-col>
        
      </el-row>
      <!-- 教室数据区域 -->
      <el-table :data="user_data" stripe style="width: 100%" border>
        ...
        <el-table-column label="编辑账号" width="180px">
          <template v-slot="scope">
            <!-- 修改按钮 -->
            <el-tooltip effect="dark" content="修改" placement="top" :enterable="false">
              <el-button type="primary" icon="el-icon-edit" size="mini" @click="edit_sccount(scope.row)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
        <el-table-column label="重置密码" width="180px">
          <template v-slot="scope">
            <!-- 修改按钮 -->
            <el-tooltip effect="dark" content="修改" placement="top" :enterable="false">
              <el-button type="primary" icon="el-icon-edit" size="mini" @click="change_pass(scope.row)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
...
      console.log(appoinmentList)
    ...
    update_account(row){
      ...
      this.$confirm('确认修改？')
          ...
          var url = '/user/edit';
          console.log("sbsbsb")
          console.log(typeof row.uid)
          this.axios.post(url,"uid="+row.uid+"&name="+row.input_name+"&e_mail="+row.input_email+"&realname="+row.input_name2+"&phone="+row.input_phonenumber).then(res => {
            console.log("看这里！")
            console.log(res.data)
    ...
    change_pass(row){
      this.$confirm('确认修改？')
          ...
          console.log("sbsbsb")
          console.log(typeof row.uid)
          this.axios.post(url,"uid="+row.uid).then(res => {
            console.log("看这里！")
            console.log(res.data)
            this.res_result = res.data
            // this.$message.error(res.msg)
      ...
      this.account_edit =[
        {
          "uid":row.uid,
          "input_name":row.name,
          "input_email":row.e_mail,
          "input_name2":row.realname,
          "input_phonenumber":row.phone,
        }
      ]
